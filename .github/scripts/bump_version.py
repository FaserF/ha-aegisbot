...
def update_files(new_version):
    ...
    with open("custom_components/aegisbot/manifest.json") as f:
        manifest = json.load(f)
    manifest["version"] = new_version
    with open("custom_components/aegisbot/manifest.json", "w") as f:
...
if __name__ == "__main__":
    ...
    bump_args = sys.argv[1].lower()
    release_args = sys.argv[2].lower()

    latest_tag = get_latest_tag()
    current_v = latest_tag.lstrip("v") if latest_tag else None

    new_v = bump_version(current_v, bump_args, release_args)
