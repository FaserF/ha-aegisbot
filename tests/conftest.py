...
def mock_api():
    ...
    with patch(
        ...
        client.async_telegram_send_poll = AsyncMock(
            return_value={"success": True, "poll_id": "p1"}
        )
        client.async_telegram_stop_poll = AsyncMock(return_value={"success": True})
        client.async_telegram_edit_message_text = AsyncMock(
            return_value={"success": True}
        )
        client.async_telegram_edit_message_caption = AsyncMock(
            return_value={"success": True}
        )
        client.async_telegram_edit_message_reply_markup = AsyncMock(
            return_value={"success": True}
        )
        client.async_telegram_delete_message = AsyncMock(return_value={"success": True})
        client.async_telegram_answer_callback_query = AsyncMock(
            return_value={"success": True}
        )
        client.async_telegram_leave_chat = AsyncMock(return_value={"success": True})
        client.async_telegram_get_me = AsyncMock(
            return_value={"id": 12345, "is_bot": True}
        )
        client.async_telegram_get_allowed_chats = AsyncMock(return_value=[])
        client.async_telegram_set_allowed_chats = AsyncMock(
            return_value={"success": True}
        )
